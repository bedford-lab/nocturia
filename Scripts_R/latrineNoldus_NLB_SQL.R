...
# What portion of activity is in home vs. latrine cage? Standard Assay.
library(Rmisc)
hc = summarySE(allTrials3_wide_sum2 %>% filter(trialType == "Record"),
          measurevar = "avg_rel_latAct", 
          groupvars = "Age")

hc$latrine = hc$avg_rel_latAct
hc$home = 1 - hc$latrine
hc = hc %>% select(Age, latrine, home)
hc = hc %>% pivot_longer(cols = c(latrine, home), names_to = "CageType", values_to = "Proportion")

ggplot(hc, aes(x = Age, y = Proportion, fill = CageType)) +
  ...
  theme_classic() +
  theme(legend.position = "none", axis.text.y=element_blank(), axis.text.x=element_blank())
ggsave("Plots/Fig2.Cage_Proportion.pdf", width=3, height=6, units="cm", dpi=1500, useDingbats=FALSE)

allTrials3_wide_sum2 %>% filter(trialType == "Record") %>% summarise(mean(avg_rel_latAct)) # 26% on average
...
# Age differences per light phase:
summary(lmer(avg_rel_latAct ~ Age + as.integer(numMice) + (1|Cage), data = allTrials3_wide_sum %>% 
               filter(Sex == "M" & Time == "light" & trialType == "Record" & Cage != "876"))) # NS
summary(lmer(avg_rel_latAct ~ Age + as.integer(numMice) + (1|Cage), data = allTrials3_wide_sum %>% 
               filter(Sex == "M" & Time == "dark" & trialType == "Record" & Cage != "876"))) # Age: 0.0887 .
...
# Age differences per light phase:
summary(lmer(avg_rel_latAct ~ Age + as.integer(numMice) + (1|Cage), data = allTrials3_wide_sum %>% 
               filter(Sex == "F" & Time == "light" & trialType == "Record" & Cage != "876"))) # Age: 0.03574 *
summary(lmer(avg_rel_latAct ~ Age + as.integer(numMice) + (1|Cage), data = allTrials3_wide_sum %>% 
               filter(Sex == "F" & Time == "dark" & trialType == "Record" & Cage != "876"))) # Age: 0.0161 *
...
## Is activity correlated with urine cover in the home cage control trials?
summary(lm(pct_cover ~ maxcum, data = cumulative.segment_max %>% filter(trialType == "Record", CageType == "latrine")))
summary(lm(pct_cover ~ maxcum, data = cumulative.segment_max %>% filter(trialType == "Ctrl", CageType == "home")))
summary(lm(pct_cover ~ maxcum, data = cumulative.segment_max %>% filter(trialType == "Ctrl", CageType == "latrine")))

hist(cumulative.segment_max$pct_cover)
hist(cumulative.segment_max$maxcum)

## Repeat the above analysis for Control trials:
# Apply the linear model to each group
# Male Data
MalePred2<- cumulative.segment_max %>%
  filter(trialType == "Ctrl" & Sex == "M" & !is.na(pct_cover)) %>%
  group_by(Age, CageType) %>%
  group_modify(~{ 
    model <- lm(pct_cover ~ maxcum, data = .x)      # Fit the model within the group
    predictions <- predict(model, se.fit = TRUE)    # Get predictions and their standard errors within the group
    augmented_data <- augment(model, .x)            # Augment the data with fitted values and standard errors
...
ggplot(MalePred2 %>% filter (Age == "Young"), aes(x = maxcum, y = pct_cover, group = CageType, colour = CageType)) +
  geom_point(size = 1.25) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_ribbon(aes(ymin = .fitted - se_fit, ymax = .fitted + se_fit), alpha = 0.3) +
  ...
  theme_classic() + 
  theme(legend.position = "none", axis.text.y=element_blank(), axis.text.x=element_blank())
ggsave("Plots/FigS2.cumLatAct_pctCover_YM.pdf", width=6, height=6, units="cm", dpi=1500, useDingbats=FALSE)

ggplot(MalePred2 %>% filter (Age == "Old"), aes(x = maxcum, y = pct_cover, group = CageType, colour = CageType)) +
  geom_point(size = 1.25) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_ribbon(aes(ymin = .fitted - se_fit, ymax = .fitted + se_fit), alpha = 0.3) +
  scale_color_manual(values=c("#00a08a", "#c6340a")) +
  ...
  theme_classic() + 
  theme(legend.position = "none", axis.text.y=element_blank(), axis.text.x=element_blank())
ggsave("Plots/FigS2.cumLatAct_pctCover_OM.pdf", width=6, height=6, units="cm", dpi=1500, useDingbats=FALSE)

M2_Y = lmer(pct_cover ~ maxcum + CageType + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "M" & Age == "Young"))
summary(M2_Y)
r.squaredGLMM(M2_Y)

summary(lmer(pct_cover ~ maxcum + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "M" & Age == "Young" & CageType == "home"))) # NS
summary(lmer(pct_cover ~ maxcum + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "M" & Age == "Young" & CageType == "latrine"))) # 0.00596 **

M2_O = lmer(pct_cover ~ maxcum + CageType + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "M" & Age == "Old"))
summary(M2_O)
r.squaredGLMM(M2_O)

# Apply the linear model to each group
# Female Data
FemalePred2<- cumulative.segment_max %>%
  filter(trialType == "Ctrl" & Sex == "F" & !is.na(pct_cover)) %>%
  group_by(Age, CageType) %>%
  group_modify(~{ 
    model <- lm(pct_cover ~ maxcum, data = .x)      # Fit the model within the group
    predictions <- predict(model, se.fit = TRUE)    # Get predictions and their standard errors within the group
...
ggplot(FemalePred2 %>% filter (Age == "Young"), aes(x = maxcum, y = pct_cover, group = CageType, colour = CageType)) +
  geom_point(size = 1.25) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_ribbon(aes(ymin = .fitted - se_fit, ymax = .fitted + se_fit), alpha = 0.3) +
  scale_color_manual(values=c("#00a08a", "#c6340a")) +
  ...
  theme_classic() + 
  theme(legend.position = "none", axis.text.y=element_blank(), axis.text.x=element_blank())
ggsave("Plots/FigS2.cumLatAct_pctCover_YF.pdf", width=6, height=6, units="cm", dpi=1500, useDingbats=FALSE)

ggplot(FemalePred2 %>% filter (Age == "Old"), aes(x = maxcum, y = pct_cover, group = CageType, colour = CageType)) +
  geom_point(size = 1.25) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_ribbon(aes(ymin = .fitted - se_fit, ymax = .fitted + se_fit), alpha = 0.3) +
  scale_color_manual(values=c("#00a08a", "#c6340a")) +
  ...
  theme_classic() + 
  theme(legend.position = "none", axis.text.y=element_blank(), axis.text.x=element_blank())
ggsave("Plots/FigS2.cumLatAct_pctCover_OF.pdf", width=6, height=6, units="cm", dpi=1500, useDingbats=FALSE)

F2_Y = lmer(pct_cover ~ maxcum + CageType + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "F" & Age == "Young"))
summary(F2_Y)
r.squaredGLMM(F2_Y)

summary(lmer(pct_cover ~ maxcum + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "F" & Age == "Young" & CageType == "home"))) # NS
summary(lmer(pct_cover ~ maxcum + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "F" & Age == "Young" & CageType == "latrine"))) # NS

F2_O = lmer(pct_cover ~ maxcum + CageType + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "F" & Age == "Old"))
summary(F2_O)
r.squaredGLMM(F2_O)

summary(lmer(pct_cover ~ maxcum + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "F" & Age == "Old" & CageType == "home"))) # NS
summary(lmer(pct_cover ~ maxcum + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Sex == "F" & Age == "Old" & CageType == "latrine"))) # NS

## Analyze both sexes together:
summary(lmer(pct_cover ~ maxcum + Sex + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Age == "Young" & CageType == "home"))) # NS
summary(lmer(pct_cover ~ maxcum + Sex + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Age == "Young" & CageType == "latrine"))) # 0.00123 **
summary(lmer(pct_cover ~ maxcum + Sex + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Age == "Old" & CageType == "home"))) # NS
summary(lmer(pct_cover ~ maxcum + Sex + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Age == "Old" & CageType == "latrine"))) # NS

## Linear Model
ControlPred<- cumulative.segment_max %>%
  filter(trialType == "Ctrl" & !is.na(pct_cover)) %>%
  group_by(Age, CageType) %>%
  group_modify(~{ 
    model <- lm(pct_cover ~ maxcum, data = .x)      # Fit the model within the group
    predictions <- predict(model, se.fit = TRUE)    # Get predictions and their standard errors within the group
...
ggplot(ControlPred %>% filter (Age == "Young"), aes(x = maxcum, y = pct_cover, group = CageType, colour = CageType)) +
  geom_point(size = 1.25) +
  ...
  theme_classic() + 
  theme(legend.position = "none", axis.text.y=element_blank(), axis.text.x=element_blank())
ggsave("Plots/FigS2.cumLatAct_pctCover_Control_Young.pdf", width=6, height=6, units="cm", dpi=1500, useDingbats=FALSE)

CtrlY = lmer(pct_cover ~ maxcum + Sex + CageType + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Age == "Young"))
summary(CtrlY)
r.squaredGLMM(CtrlY)

ggplot(ControlPred %>% filter (Age == "Old"), aes(x = maxcum, y = pct_cover, group = CageType, colour = CageType)) +
  geom_point(size = 1.25) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_ribbon(aes(ymin = .fitted - se_fit, ymax = .fitted + se_fit), alpha = 0.3) +
  scale_color_manual(values=c("#00a08a", "#c6340a")) +
  ...
  theme_classic() + 
  theme(legend.position = "none", axis.text.y=element_blank(), axis.text.x=element_blank())
ggsave("Plots/FigS2.cumLatAct_pctCover_Control_Old.pdf", width=6, height=6, units="cm", dpi=1500, useDingbats=FALSE)

CtrlO = lmer(pct_cover ~ maxcum + Sex + CageType + (1|Cage), data = cumulative.segment_max %>% filter(trialType == "Ctrl" & Age == "Old"))
summary(CtrlO)
r.squaredGLMM(CtrlO)
